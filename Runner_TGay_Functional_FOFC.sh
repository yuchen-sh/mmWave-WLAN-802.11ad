...
#         do
                for ii in $(seq 1 $i)
                do
                        echo "ii: $ii " >>  2021-5-6-TGay-Functional-FOFC.txt
                        for clientRS in $(seq 1 $num_client)
                        do
                                ./waf --run "scratch/TGay_Functional_eval_living_room --ii=${ii} --clientRS=${clientRS}" 2>> 2021-5-6-TGay-Functional-FOFC.txt
                        done
                done
        # done
# done
