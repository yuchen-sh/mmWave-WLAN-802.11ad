...
// get on_time from EmpiricalRandomVariable
void 
OnOffHelper::GetOnTimeFromEmpRand (Ptr<Node> n, Ptr<EmpiricalRandomVariable> x_e)
{
  Ptr<Node> node;
  node = n;
  for (uint32_t j = 0; j < node->GetNApplications (); j++)
...
// get off_time from EmpiricalRandomVariable
void 
OnOffHelper::GetOffTimeFromEmpRand (Ptr<Node> n, Ptr<EmpiricalRandomVariable> x_e)
{
  Ptr<Node> node;
  node = n;
  for (uint32_t j = 0; j < node->GetNApplications (); j++)
