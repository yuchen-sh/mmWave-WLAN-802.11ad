...
(Note: We implemented and modeled exactly the simulation scenarios, traffic models, and link budget parameters for point-to-point link simulation. The directional antenna model used in this repo is slightly differently from the standardized antennas as defined in the document, where an analytical codebook is implemented based on Gaussian directional pattern formula to define the shape of beam patterns. We have not modeled the hardware impairments and PHY abstractions as mentioned in the document.)
