...
#         do
                for ii in $(seq 1 $i)
                do
                        echo "ii: $ii " >>  2021-4-21-TGad-Functional-FOFC-SV.txt
                        for clientRS in $(seq 1 $num_client)
                        do
                                ./waf --run "scratch/TGad_Functional_eval_living_room_SVChannel --ii=${ii} --clientRS=${clientRS}" 2>> 2021-4-21-TGad-Functional-FOFC-SV.txt
                        done
                done
        # done
# done
